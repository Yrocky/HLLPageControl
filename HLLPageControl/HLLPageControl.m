...
static CGFloat const kPageControlDotViewSize = 6.0f;
static CGFloat const kPageControlRoundedRectangleViewWidth = 25.0f;
...
- (void)setNumberOfPages:(NSInteger)numberOfPages{
    ...
    NSAssert(numberOfPages < 10, @"Your numberOfPages is bigger then ten ,It's not cool.");
...
- (CGRect) calculationDotViewRectWithNumberOfPages:(NSInteger)numberOfPages{
    ...
    CGSize size = CGSizeMake(width, kPageControlDotViewSize);
    
    CGPoint point = CGPointMake(CGRectGetWidth(self.bounds) / 2 - size.width / 2,
                                CGRectGetHeight(self.bounds) - kPageControlDotViewSize - 5);
...
- (void)setupPageControlCurrentPage:(NSInteger)currentPage animation:(BOOL)animation{
    ...
    NSAssert(currentPage < self.numberOfPages, @"Your CurrentPage is bigger then The Max number of pages");
...
- (void)setupPageControlCurrentPage:(NSInteger)currentPage{
    ...
    [self setupPageControlCurrentPage:currentPage animation:NO];
}
